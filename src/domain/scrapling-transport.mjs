...
const PYTHON_DYNAMIC_FETCH_SCRIPT = String.raw`
import json
import sys
from pathlib import Path
from scrapling.fetchers import DynamicFetcher

url, body_path, metadata_path, timeout_ms, wait_ms, network_idle, wait_selector = sys.argv[1:8]
response = DynamicFetcher.fetch(
    url,
    headless=True,
    real_chrome=True,
    network_idle=network_idle == "true",
    load_dom=True,
...
export async function fetchViaScraplingDynamic(requestedUrl, options = {}) {
  const directory = await mkdtemp(join(tmpdir(), 'fitappliance-scrapling-dynamic-'));
  const bodyPath = join(directory, 'body');
  const metadataPath = join(directory, 'metadata.json');
  try {
    ...
    const pythonBinary = await resolveScraplingPython(options);
    const execDynamic = options.execFileImpl ?? execFile;
    await execDynamic(pythonBinary, [
      '-c', PYTHON_DYNAMIC_FETCH_SCRIPT,
      new URL(requestedUrl).toString(),
      bodyPath,
      metadataPath,
      String(timeoutMs),
      String(waitMs),
      String(options.networkIdle ?? true),
      String(options.waitSelector ?? ''),
    ], {
      timeout: options.processTimeoutMs ?? (timeoutMs * 4 + 10000),
      maxBuffer: 2 * 1024 * 1024,
      env: { ...process.env, PYTHONUNBUFFERED: '1' },
    });
    const [metadata, bytes] = await Promise.all([
    ...
    if (!Number.isInteger(metadata.status) || metadata.status < 200 || metadata.status >= 300) {
      throw new Error(`scrapling_dynamic_http_${metadata.status || 'unknown'}`);
    }
    if (metadata.byteSize !== bytes.length) throw new Error('Scrapling dynamic body size metadata mismatch');
    if (bytes.length > (options.maximumBytes ?? Number.POSITIVE_INFINITY)) {
      throw new Error('Scrapling dynamic artifact size outside limits');
    }
    const requested = new URL(requestedUrl).toString();
    const finalUrl = new URL(metadata.finalUrl, requested).toString();
    return {
