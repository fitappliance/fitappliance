...
export function canApproveEvidence(input) {
  ...
  const identityApproved = record.identityMatch === 'exact'
    || (record.identityMatch === 'alias' && record.aliasApproved === true);
  if (!identityApproved) {
...
export function createFieldEvidence(input) {
  ...
  const approval = canApproveEvidence(record);
...
export function evidenceLevel(records) {
  if (!Array.isArray(records)) {
    throw new TypeError('evidence records must be an array');
  }

  const approvedFields = new Set(records
    .filter((record) => record?.status === 'approved' && canApproveEvidence(record).approved)
