...
function normalizeAdvisoryChecks(checks) {
  if (!Array.isArray(checks)) {
    throw new TypeError('advisory checks must be an array');
  }
  const seen = new Set();
  return checks.map((input, index) => {
    ...
    if (!CHECK_STATUSES.has(status)) {
      throw new RangeError(`unsupported advisory check status: ${status}`);
    }
    const id = requireString(check.id, `advisory check ${index} id`);
    if (id.startsWith('installation_')) {
      throw new RangeError(`reserved advisory check id: ${id}`);
    }
    if (seen.has(id)) {
      throw new RangeError(`duplicate advisory check id: ${id}`);
    }
    seen.add(id);
    return {
      id,
...
export function evaluateFit(input) {
  ...
  if (!EVIDENCE_LEVELS.has(level)) {
    throw new RangeError(`unsupported evidence level: ${level}`);
  }
  ...
  const required = {
    ...
    depthMm: sumKnown([
      depthMm,
      finiteOrNull(installation.rearMm, 'installation.rearMm', { allowZero: true }),
      finiteOrNull(installation.frontMm, 'installation.frontMm', { allowZero: true }),
    ]),
  };
  const available = {
  ...
  let outcome;
  if (
    axisChecks.some((check) => check.status === 'FAIL')
