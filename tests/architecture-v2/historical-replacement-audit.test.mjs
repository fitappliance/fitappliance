...
test('only the active-release publisher can write runtime and replacement files', () => {
  ...
  assert.equal(scripts['refresh:historical-reference'], undefined);
  assert.match(activePublisher, /publishRuntimeProjection/);
  assert.match(activePublisher, /publishHistoricalReference/);
});
