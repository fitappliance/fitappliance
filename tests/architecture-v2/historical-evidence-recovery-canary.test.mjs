...
import { validateHistoricalEvidenceRecoveryResults } from '../../src/domain/historical-evidence-recovery-contract.mjs';
...
const results = JSON.parse(await readFile(new URL(
  '../../data/architecture-v2/reviews/automated/historical-evidence-recovery-results.json',
  import.meta.url,
), 'utf8'));
...
test('real WD8560F1 canary binds exact official PDF, MinerU claims and dimensions-only geometry', () => {
  ...
  const outcome = results.outcomes.find((row) => row.targetId === fixture.targetId);
