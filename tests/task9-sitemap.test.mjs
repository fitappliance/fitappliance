...
test('task 9.1 sitemap: includes static pages even when brand index is empty', async () => {
  ...
  assert.equal(nodes[2].loc, 'https://www.fitappliance.com.au/privacy');
  assert.equal(nodes[3].loc, 'https://www.fitappliance.com.au/privacy-policy');
  assert.equal(nodes[4].loc, 'https://www.fitappliance.com.au/terms');
  assert.equal(nodes[5].loc, 'https://www.fitappliance.com.au/contact');
});
...
test('task 9.1 sitemap: keeps static URLs first and sorts brand URLs by category then brand', async () => {
  ...
  assert.deepEqual(locs.slice(0, STATIC_PAGES.length), [
    ...
    'https://www.fitappliance.com.au/privacy',
