...
function buildComparisonPageHtml({
  ...
  const sampleItemsA = modelSamplesA.map((sample) => {
    ...
    const buyHtml = affiliate
      ? `<br><a href="${escHtml(affiliate.url)}" target="_blank" rel="sponsored nofollow noopener">Buy at ${escHtml(affiliate.providerName)} →</a><small style="display:block;color:#6b6b6b;margin-top:4px">${escHtml(affiliate.disclosureText)} <a href="/affiliate-disclosure" style="color:#A34F22">Disclosure</a></small>`
      : retailLink
  ...
  <main>
    ...
    <section style="margin:32px 0;padding:16px 24px;background:#f5f2ec;border-radius:8px;border:1px solid #e0d9ce">
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <a href="/brands/${escHtml(slugify(brandA))}-${escHtml(categoryMeta.slug)}-clearance" style="display:inline-flex;align-items:center;min-height:44px;font-size:13px;color:#A34F22;text-decoration:none">${escHtml(displayBrandA)} ${escHtml(categoryMeta.labelSingular)} clearance specs →</a>
        <a href="/brands/${escHtml(slugify(brandB))}-${escHtml(categoryMeta.slug)}-clearance" style="display:inline-flex;align-items:center;min-height:44px;font-size:13px;color:#A34F22;text-decoration:none">${escHtml(displayBrandB)} ${escHtml(categoryMeta.labelSingular)} clearance specs →</a>
      </div>
    </section>
    ${alsoViewedComparisons.length > 0 ? `<section style="margin:20px 0;padding:16px 24px;background:#f5f2ec;border-radius:8px;border:1px solid #e0d9ce">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ${alsoViewedComparisons.map((row) => `<a href="/compare/${escHtml(row.slug)}" style="display:inline-flex;align-items:center;min-height:44px;padding:6px 14px;border:1px solid #e0d9ce;border-radius:20px;font-size:13px;color:#A34F22;text-decoration:none;background:#fff">${escHtml(row.label)}</a>`).join('\n        ')}
      </div>
    </section>` : ''}
    <section style="margin:20px 0;padding:16px 24px;background:#f5f2ec;border-radius:8px;border:1px solid #e0d9ce">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ${GUIDE_HUB_LINKS.map((row) => `<a href="${escHtml(row.url)}" style="display:inline-flex;align-items:center;min-height:44px;padding:6px 14px;border:1px solid #e0d9ce;border-radius:20px;font-size:13px;color:#A34F22;text-decoration:none;background:#fff">${escHtml(row.label)}</a>`).join('\n        ')}
      </div>
    </section>
    <footer style="margin-top:16px;padding-top:14px;border-top:1px solid #e0d9ce;font-size:13px;color:#6b6b6b">
      <a href="/methodology" style="display:inline-flex;align-items:center;min-height:44px;color:#A34F22;text-decoration:none">Methodology</a> ·
      <a href="/about/editorial-standards" style="display:inline-flex;align-items:center;min-height:44px;color:#A34F22;text-decoration:none">Editorial standards</a>
    </footer>
  </main>
  <script>
...
async function generateComparisonPages(options = {}) {
  ...
  for (const pair of pairs) {
    ...
    const record = {
      ...
      lastUpdated: contentModifiedTime,
      filePath
    };
