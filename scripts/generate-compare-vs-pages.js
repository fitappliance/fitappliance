...
<body>
  <main class="compare-static-page">
    ...
    <section class="comparison-detail">
      ...
      <p><button type="button" data-fit-query="/?cat=${escHtml(row.cat)}&intent=compare&brand=${encodeURIComponent(brandA)}&vs=${encodeURIComponent(brandB)}" onclick="window.location.href=this.dataset.fitQuery" style="display:inline-flex;align-items:center;min-height:44px;border:0;background:transparent;color:#b55a2c;font:inherit;font-weight:700;cursor:pointer;padding:0">Run a live cavity comparison for ${escHtml(brandA)} vs ${escHtml(brandB)} →</button></p>
    </section>
