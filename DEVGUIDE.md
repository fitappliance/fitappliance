...
- Create `scripts/circuit-breaker.js` — 断路器逻辑（见下方 Task 3.1）
...
**实现规范**（在 `scripts/sources/fetch-utils.js` 中统一封装）：

```js
// scripts/sources/fetch-utils.js
...
**三条熔断规则**（`scripts/circuit-breaker.js`）：
...
- `npm run validate` validates current JSON files and exits 0
